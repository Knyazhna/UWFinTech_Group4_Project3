...
contract Staking {
    ...
    //Constructor
    constructor(RewardToken Reward) {
    ...
    function deposit_stake(uint256 _stake) internal {
        ...
        require(reward.balanceOf(tx.origin) >= _stake.mul(1e18) && _stake != 0, "Not enough balance to stake");
        ...
        reward.approveFromContract(tx.origin, address(this), _stake_digits);
        reward.transferFrom(tx.origin,address(this), _stake_digits);
    ...
    function withdraw_stake() internal UpdateReward() {
        ...
        reward.approveFromContract(address(reward),address(this),stake[msg.sender].stake_amount_digits);
        reward.transferFrom(address(reward),tx.origin,stake[msg.sender].stake_amount_digits);
    ...
    function withdraw_reward() public UpdateReward() {
        ...
        if (block.timestamp < stake[msg.sender].time_end) {
            reward.approveFromContract(address(reward), address(this), staking_reward[msg.sender].div(2));
            reward.transferFrom(address(reward),tx.origin,staking_reward[msg.sender].div(2));
        ...
        if (block.timestamp > stake[msg.sender].time_end  && block.timestamp <= (stake[msg.sender].time_end + checked_in_period)) {
            reward.approveFromContract(address(reward), address(this), staking_reward[msg.sender]);
            reward.transferFrom(address(reward),tx.origin,staking_reward[msg.sender]);
    ...
    function reward_share() internal returns(uint256 _digits,uint256 _rounded) {
        ...
        if (Checked.length != 0) {
            ...
            for (uint checked_id = 0; checked_id < Checked.length; checked_id += 1) {
                 reward.approveFromContract(address(reward),address(this),digits);
                 reward.transferFrom(address(reward),Checked[checked_id],digits);
        ...
        if (isStaking.length != 0) {
            for (uint id = 0; id < isStaking.length; id += 1) {
                ...
                reward.approveFromContract(address(reward),address(this),(9 * (stake[isStaking[id]].stake_amount_digits / 10)));
                reward.transferFrom(address(reward),isStaking[id],(9 * (stake[isStaking[id]].stake_amount_digits / 10)));
